...
def seed_demo_data():
    ...
    try:
        ...
        # 2. Remove any Eagle Highway demo projects if they exist
        demo_projs = db.query(models.Project).filter(models.Project.name.like("%Eagle Highway%")).all()
        for dp in demo_projs:
            db.delete(dp)
        db.commit()
        if demo_projs:
            print(f"[DEMO] Removed {len(demo_projs)} demo project(s).")
    except Exception as e:
        print(f"[DEMO] Notice during seed check: {e}")
    finally:
        db.close()
